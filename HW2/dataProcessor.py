...
class dataProcessor:
    ...
    def saveMatrixToFile(self):
        self.dataFrame.to_csv("trace.csv")
    ...
    def drawTrace(self, tid = None, saveName = "All"):
        ...
        if 0 == cmp("All", saveName):
            resFile = open("Result/File/All.js", 'a')
        else:
            resFile = open("Result/File/data.js", 'a')
        ...
        for index in range(dataSize):
            ...
            if (tid is None) or (self.__data.Tid[index] in tid):
                ...
                if index == dataSize - 1 or self.__data.Tid[index] != self.__data.Tid[index + 1]:
                    plt.plot(traceItemY, traceItemX)
                    a += 1
                    if not flag:
                        resFile.write("[")
                        flag = True
                    ...
                    for i, X in enumerate(traceItemX):
                        if i == len(traceItemX)-1:
                            foo = "]"
                        else:
                            foo = "],"
                        resFile.write("[" +str(traceItemX[i]) + ", " + str(traceItemY[i]) + foo)
        ...
        #保存并把结果输出到文件
        plt.yticks(np.linspace(31.16, 31.30, 8))
        plt.xticks(np.linspace(121.376, 121.560, 10))
